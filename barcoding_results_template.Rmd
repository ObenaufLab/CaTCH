...
# params <- list(counts = opt$countsFile, summaries = opt$summariesFile, hammdist=opt$hammingFile, samples = opt$samples, outpref = file.path(opt$resultsDir, sub('.txt|.tsv', '', basename(opt$countsFile))), topdf = opt$topdf, refsamps=opt$refsamps, abund_thresh=opt$abund_thresh, count_thresh=opt$count_thresh )
...
summaries <- summaries[, c("row_ID", sampleNames), with=FALSE][row_ID %in% c("BCUnmatched", "EmptyVector", "SampleUnknown", "SpikeIn", sampleNames)]
if (!is.null(params$hammdist)) {
...
counts_tidy <- melt(counts[, 2:length(counts)], id.vars='bc_id', variable.name="Sample", value.name="Reads")
...
tmp <- rbind(tmp, cbind(fillup, data.frame(bc_id=0, Reads=0, count_Good=NA, prop_Top=FALSE, treatment=NA, is_Shared=NA, sharedFill='fillup')) )
...
names(elemcol) <- elem
elemcol["notshared"] <- "grey90"
elemcol["fillup"] <- "black"
