...
all_tidy <- Reduce(function(a,b){merge(a, b, by="bc_id", all=TRUE)}, 
			 list(all_tidy, 
			 		  sharedBarcodes[, .(bc_id, nShared)],
			 		  sharedBarcodesGrp[, .(bc_id, nSharedGrp)],
			 		  sharedBarcodesTrt[, .(bc_id, nSharedTrt)]) )
...
p1 <- ggplot(unique(all_tidy[cum_rank>0, .(bc_id, cum_rank)]), aes(x=cum_rank)) +
	geom_histogram(bins=max(all_tidy$cum_rank)/2) +
	labs(x="Cumulative rank", y="Barcodes", title="Potential interest") +
	theme_light()
