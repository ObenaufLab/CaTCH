...
# params <- list(counts = opt$countsFile, summaries = opt$summariesFile, hammdist=opt$hammingFile, samples = opt$covars, outpref = file.path(opt$resultsDir, sub('.txt|.tsv', '', basename(opt$countsFile))), topdf = opt$topdf, refsamps=opt$refsamps, abund_thresh=opt$abund_thresh, count_thresh=opt$count_thresh, extra_bc= opt$extrabc, hammdist = opt$hammdist )
...
if (length(samples_tidy) == 3) {
	setnames(samples_tidy, c("name", "treatment", "colour"))
} else {
	setnames(samples_tidy, c("tag", "name", "treatment", "colour"))
	samples_tidy[, tag := NULL]
}
...
pcount <- ggplot(data=droplevels(count_summary[(Type != "top_Barcodes"), ]), aes(x=Sample, y=Count, fill=Type))  +
...
This section identifies the abundance of each selected barcode (white diamonds) against the distribution of all barcodes (violin/boxplots).
...
```{r enrichment_shared}
pviolas <- lapply(sharedBarcodes$bc_id, function(bc) {
