...
```{r libraries, result="hide", message=FALSE}
# Data
...
pbcdist2 <- ggplot(all_tidy[Reads > 0, ], aes(x=Reads, colour=Sample, fill=Sample)) +
	...
	scale_colour_manual(values=mycolours) +
	scale_fill_manual(values=mycolours) +
	labs(x='Reads', subtitle='only detected BCs') +
	theme_classic() +
	theme(panel.grid.minor=element_blank(),
				legend.position='none')
...
all_tidy <- merge(all_tidy, samples_tidy, by="Sample")
```

```{r overview, fig.height=8, fig.width=10}
# Make tidy summary of counts.
count_summary <- Reduce(function(x, y){merge(x,y, by='Sample')},
			list(all_tidy[, sum(Reads), by=Sample],
...
pcount <- ggplot(data=melt(count_summary, id.vars=c("Sample"), value.name="Count", variable.name="Type"), 
								 aes(x=Sample, y=Count, fill=Type))  +
            facet_grid(Type ~ ., scales = "free") +
            geom_bar(stat='identity', width=0.7, size=0.2) +
            scale_fill_manual(values = c(BCs_top = "mediumorchid1",
            														 BCs_fltr = "mediumorchid3",
            														 BCs = "mediumorchid4",
...
sharedBarcodes <- Reduce(rbind,
			 apply(sharedBarcodes, 1, function(x){
			 	all_tidy[prop_Top & bc_id==x['bc_id'], paste(Sample, collapse = " ; "), by=bc_id]
			 	})
)
setnames(sharedBarcodes, c("bc_id", "Samples"))

sharedBarcodesGrp <- Reduce(rbind,
			 apply(sharedBarcodesGrp, 1, function(x){
			 	all_tidy[prop_Top & bc_id==x['bc_id'] & Group==x['Group'], paste(Sample, collapse = " ; "), by=c("Group", "bc_id")]
			 	})
)
setnames(sharedBarcodesGrp, c("group", "bc_id", "Samples"))
...
sharedBarcodesTrt <- Reduce(rbind,
			 apply(sharedBarcodesTrt, 1, function(x){
			 	all_tidy[prop_Top & bc_id==x['bc_id'] & Treatment==x['Treatment'], paste(Sample, collapse = " ; "), by=c("Treatment", "bc_id")]
			 	})
)
setnames(sharedBarcodesTrt, c("treatment", "bc_id", "Samples"))
...
all_tidy[, is_SharedGrp := bc_id %in% sharedBarcodesGrp$bc_id]
all_tidy[, is_SharedTrt := bc_id %in% sharedBarcodesTrt$bc_id]
```
...
if(length(dropped) > 0)
  tmp <- rbind(tmp, data.frame(Sample=dropped, bc_id=NA_character_, Reads=0, Proportion=0, count_Good=NA, refcount_Good=NA,
  														 ...
  														 is_Shared=NA, is_SharedGrp=NA, is_SharedTrt=NA, sharedFill=NA_integer_) )
...
tmp <- rbind(tmp, cbind(fillup, data.frame(bc_id=NA_character_, Reads=0, count_Good=NA,refcount_Good=NA, prop_Top=FALSE,
																					 ...
																					 is_Shared=NA, is_SharedGrp=NA, is_SharedTrt=NA, sharedFill='fillup')) )
...
if(length(dropped) > 0)
  tmp <- rbind(tmp, data.frame(Sample=dropped, bc_id=0, Reads=0, Proportion=0, count_Good=NA, refcount_Good=NA,
  														 prop_Top=FALSE, Tag=NA_character_, Group=NA_character_, Treatment=NA_character_,
  														 is_Shared=NA, is_SharedGrp=NA, is_SharedTrt=NA, sharedFill=NA_integer_, sharedFillGrp=NA_integer_) )
fillup <- tmp[, .(Sample, Proportion)] [, 1-sum(Proportion), by=Sample]
setnames(fillup, c("Sample", "Proportion"))
tmp <- rbind(tmp, cbind(fillup, data.frame(bc_id=-Inf, Reads=0, count_Good=NA,refcount_Good=NA, prop_Top=FALSE,
																					 Tag=NA_character_, Group=NA_character_, Treatment=NA_character_,
																					 is_Shared=NA, is_SharedGrp=NA, is_SharedTrt=NA, sharedFill=NA_character_, 
																					 sharedFillGrp='fillup')) )
...
if(length(dropped) > 0)
  tmp <- rbind(tmp, data.frame(Sample=dropped, bc_id=0, Reads=0, Proportion=0, count_Good=NA, refcount_Good=NA,
  														 prop_Top=FALSE, Tag=NA_character_, Group=NA_character_, Treatment=NA_character_,
  														 is_Shared=NA, is_SharedGrp=NA, is_SharedTrt=NA, sharedFill=NA_integer_, 
  														 sharedFillGrp=NA_integer_, sharedFillTrt=NA_integer_) )
fillup <- tmp[, .(Sample, Proportion)] [, 1-sum(Proportion), by=Sample]
setnames(fillup, c("Sample", "Proportion"))
tmp <- rbind(tmp, cbind(fillup, data.frame(bc_id=-Inf, Reads=0, count_Good=NA,refcount_Good=NA, prop_Top=FALSE,
																					 Tag=NA_character_, Group=NA_character_, Treatment=NA_character_,
																					 is_Shared=NA, is_SharedGrp=NA, is_SharedTrt=NA, sharedFill=NA_character_,
																					 sharedFillGrp=NA_integer_, sharedFillTrt='fillup')) )
...
# Interesting barcodes

"Interesting-ness" is loosely based on a barcode meeting the abundance thresholds and on how many samples have that barcode enriched in common. This score is meant to help in identifying recurring enriched barcodes, but it is not intended as a definitive criterion, In addition, the values are not comparable across analyses of different samples. The cutoff for interesting-ness is rather arbitrary and based on the number of barcodes that achieve each score: The cutoff is set at the highest score value that is achieved by 10 or more different barcodes.

```{r table2}
# Collect flags to put in wide format
DF <- all_tidy[, 
							 .(cnt = sum(count_Good), 
								 pct = sum(prop_Top), 
								 sh = sum(is_Shared), 
								 shg = sum(is_SharedGrp), 
								 sht = sum(is_SharedTrt)) , 
							 by=c("bc_id", "Group", "Treatment")]
DF[, interesting := rowSums(DF[, c(4:5, 7:8), with=FALSE])]  # is_Shared mixes all samples, treated and untreated. Barcodes there are probably the opposite of interesting.

all_tidy <- merge(all_tidy, DF[, list(interesting = sum(interesting)), by=bc_id], by="bc_id")


# Spread abundances to wide format
DF <- dcast(all_tidy, bc_id + interesting ~ Sample, value.var=c("Reads", "Proportion"))
DF <- merge(counts[, .(bc_id, barcode)], DF, by="bc_id")
setorder(DF, -interesting)


fwrite(DF, file=paste0(params$outpref, '_data.tsv'), sep="\t", quote=FALSE)


tmp <- DF[, .N, by=interesting]
tmp[, cs := cumsum(N)]
print( DF[ interesting >= tail(tmp[N<10, interesting], 1), .(bc_id, barcode, interesting)] )

interestingbcs <- DF[ interesting >= tail(tmp[N<10, interesting], 1), bc_id]

p1 <-	ggplot(DF, aes(x=interesting)) +
		geom_histogram(bins=max(DF$interesting, na.rm=TRUE)) +
		labs(y="Barcodes") +
		theme_light()
...
## Expression heatmap of interesting barcodes.
...
```{r enrichment_shared}
pviolas <- lapply(interestingbcs, function(bc) {
  # bc <- interestingbcs[1]
